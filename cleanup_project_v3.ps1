# Script de nettoyage v3 pour le projet cccomputer
# Règles: Supprimer certains fichiers/dossiers, DÉPLACER les .md dans _docs_archive/
# Usage:
#   Dry-run: powershell -ExecutionPolicy Bypass -File .\cleanup_project_v3.ps1 -WhatIf
#   Exécution: powershell -ExecutionPolicy Bypass -File .\cleanup_project_v3.ps1
#   Avec chemin: powershell -ExecutionPolicy Bypass -File .\cleanup_project_v3.ps1 -ProjectRoot "C:\path\to\project"
...
if ($ProjectRoot) {
    ...
    if (-not (Test-Path $projectRoot)) {
        Write-Host "[ERREUR] Le chemin spécifié n'existe pas: $projectRoot" -ForegroundColor Red
        exit 1
    }
} else {
...
Write-Host "[INFO] Répertoire du projet: $projectRoot" -ForegroundColor Cyan
...
# Utilisé pour scanner les fichiers avec Get-ChildItem
$fileExtensionsToDelete = @(
    "log", "tmp", "bak", "old", "swp", "map", 
    "doc", "docx", "odt", "rtf"
)

# Patterns de fichiers à supprimer (avec wildcards)
...
# Scanner les fichiers à supprimer en utilisant $fileExtensionsToDelete
Write-Host "[SCAN] Recherche des fichiers à supprimer (extensions)..." -ForegroundColor Yellow
...
Write-Host "[SCAN] Recherche des fichiers avec patterns (*.spec.*, *.test.*)..." -ForegroundColor Yellow
foreach ($pattern in $filePatternsToDelete) {
    $specFiles = Get-ChildItem -Path $projectRoot -Recurse -File -ErrorAction SilentlyContinue | Where-Object {
        $_.Name -like $pattern -and -not (Is-ProtectedPath $_.FullName)
    }
    foreach ($file in $specFiles) {
        if ($itemsToDelete.Files -notcontains $file.FullName) {
            $itemsToDelete.Files += $file.FullName
            $type = "spec/test"
...
if ($WhatIf) {
    Write-Host "[MODE WHATIF] Aucune modification ne sera effectuée" -ForegroundColor Yellow
}
...
# Si mode WhatIf, arrêter ici
if ($WhatIf) {
    Write-Host "`n========================================" -ForegroundColor Yellow
    Write-Host "MODE WHATIF - AUCUNE MODIFICATION EFFECTUEE" -ForegroundColor Yellow
    Write-Host "========================================" -ForegroundColor Yellow
    Write-Host "Pour exécuter réellement, relancez sans -WhatIf" -ForegroundColor Cyan
    exit 0
}
...
$report += "`n---`n`n*Rapport généré automatiquement par cleanup_project_v3.ps1*"
